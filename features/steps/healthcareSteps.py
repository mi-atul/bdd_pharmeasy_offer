...
@when('Click on Healthcare button')
def all_btn(context):
    context.driver.find_element("xpath", '//div[text()="Healthcare"]').click()


@when('Click on Copy Code button')
def copy_btn(context):
    try:
        context.driver.find_element("xpath", '//div[@class="OffersCard_container__2g9dg"][2]//span[text()="Copy Code"]').click()

    except:
        context.driver.find_element("xpath", '//div[@class="OffersCard_container__2g9dg"][2]//span[text()="Continue"]').click()


@when('Click on Coupon')
def copy_btn(context):
    try:
        context.driver.find_element("xpath", '//div[@class="OffersCard_container__2g9dg"][2]').click()
    except:
        context.driver.back()
        context.driver.find_element("xpath", '//div[@class="OffersCard_container__2g9dg"][2]').click()
        time.sleep(5)
...
def copy_btn(context):
    try:
        ...
        context.driver.back()
        context.driver.find_element("xpath", '//button[text()="COPY CODE"]').click()
        assert False, "Coupon's Copy Code button is not working properly"
        time.sleep(5)
...
@then('Healthcare mode offers should be displayed and User is able to click on Copy Code button and offer code Pop-Up message is displayed')
